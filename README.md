...
- 🚧 **Phase 7 (Step 8)** — 전체 UI 리디자인(§14): 편집기와 제품 셸이 **하나의 디자인 토큰 팔레트**를 공유(앱 배경/표면/경계/텍스트, 기획서=인디고·이미지=바이올렛 강조, 반경 12/8px, Pretendard 계열 폰트). 공통 버튼·입력·카드 규격화, 선택은 **얇은 강조 링 + 은은한 글로우**, 상단 앱 바 64px, 정직한 빈 상태·`prefers-reduced-motion` 대응
...
- 순수 스타일·마크업 정렬만 변경 — 기능/로직/데이터 구조는 그대로(테스트 195개 유지)
