...
export interface StudioJob {
  ...
  /**
   * 페이지 id → 그 페이지가 어느 배너 규격인가 (배너 Patch §5).
   *
   * 배너는 **페이지로 산다.** 그래야 결과 화면의 편집이 통째로 따라온다 — 끌어
   * 옮기기, 크기 조절, 앞뒤, 삭제, 확대. 새로 만들 화면이 거의 없다.
   *
   * 다만 페이지 탭에는 나오지 않는다. 작성자에게 페이지 탭은 "이벤트 페이지가 몇
   * 장인가"를 뜻하는 자리이고, 이벤트 페이지가 두 장인 경우는 없다고 확인받았다.
