...
describe('§9 블록마다 미리 주문한다', () => {
  ...
  it('주문이 작업 파일에 남고, 빈 값은 지워지며, 예전 파일은 빈 값으로 읽힌다', async () => {
    ...
    // (저장된 값을 직접 본다 — 검사 환경은 `blockOrderOf`에 시험 글꼴을 채워 넣는다.)
    expect(job.blockOrders?.blk_t1).toBeUndefined()
...
describe('§16 완성본이 가운데를 다 쓴다', () => {
  it('기획서는 접혀 있고, 배율은 완성본의 것이며, 판의 폭이 그 배율을 탄다', async () => {
    ...
    fireEvent.pointerDown(container.querySelector('.result-objects')!, { button: 0, clientX: 10, clientY: 10 })
