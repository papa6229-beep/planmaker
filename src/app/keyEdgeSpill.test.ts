/**
 * 오려낸 글자의 가장자리 (자주색 테두리 Patch).
 *
 * 모델은 투명한 배경으로 그려 주지 않는다. 그래서 문구는 **단색 마젠타 위에**
 * 그려져 오고, 그 마젠타를 여기서 걷어 낸 뒤 배경 위에 얹는다. 손검수에서 나온
 * 지저분한 외곽선은 전부 이 걷어 내기에서 생긴 것이었다 — 모델이 그린 그림은
 * 그 자체로 깨끗했다. 웹에서 GPT에게 한 장을 통째로 받으면 이런 일이 없는 것도
 * 같은 이유다. 거기엔 오려내는 단계가 없다.
 *
 * 세 가지가 겹쳐 있었고, 이 파일은 그 셋을 각각 붙든다.
 *
 *  1. **잘라내는 자가 디자인 팔레트를 지나갔다.** 판정이 "마젠타에서 거리 110
 *     이내면 지운다" 하나였는데, 파스텔·핑크 디자인의 분홍 외곽선이 거리 106,
 *     자홍빛 보라가 74다. 분홍 획은 안쪽이 진하고 가장자리로 갈수록 연해지므로
 *     그러데이션이 경계선을 넘나들며 **획을 군데군데 뚫어 놓았다.** 부드럽게
 *     번진 줄 알았던 가장자리는 사실 글자에 뚫린 구멍이었다. (§26-2 분홍)
 *
 *  2. **판정이 0 아니면 255뿐이었다.** 반쯤 덮인 픽셀이 통째로 한쪽에 몰리니
 *     모델이 그린 안티앨리어싱이 버려지고 글자가 1비트 경계로 끝났다. 상자
 *     크기로 줄여 붙이면 계단처럼 보인다. (§26-2 옅은 겹)
 *
 *  3. **남은 띠의 색을 원래 색 없이 풀었다.** 흰 글자에서는 맞았지만 색 있는
 *     글자에서 엉뚱한 색이 나왔다. 이제 가장자리 바로 안쪽 픽셀에게 원래 색을
 *     묻는다. (§26-1)
 *
 * 지금은 걷어 내기가 **바깥에서 이어진 자리**만 뜯어 들어가고, 갇힌 바탕만 좁은
 * 자로 따로 잡는다. 그래서 글자 안쪽 색은 얼마나 키 색을 닮았든 안전하다.
 */
