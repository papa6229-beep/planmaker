 ...
 * 사라지는지 셀 수 있어야 한다.
 *
 * 표면마다 지우는 것이 다르다. 작성기는 기획서와 그 이미지를, 작업판은 Studio
 * 작업과 그 제품 이미지를 지운다. 이미지 blob은 셋(기획서·전달 스냅숏·Studio
 * 작업)이 함께 쓰는 하나의 풀이므로, 어느 쪽을 지우든 **남은 쪽이 아직 쓰는
 * 것은 지우지 않는다** — 정리는 언제나 "아무도 참조하지 않는 것"만 걷어낸다.
 *
...
import { allStudioAssetIds, clearAllStudioJobs, loadStudioJob, STUDIO_JOB_ID } from './studioStore'
import { studioAssetIds } from '../domain/studioJob'
...
export async function countWriterStorage(): Promise<StorageCount> {
  ...
  for (const id of await allStudioAssetIds()) owned.delete(id)
...
/** 작업판에서 지워질 것: Studio 작업과, 그 작업만 쓰는 제품 이미지. */
export async function countStudioStorage(): Promise<StorageCount> {
  const job = await loadStudioJob(STUDIO_JOB_ID)
  if (job === null) return { briefs: 0, images: 0 }
  const owned = new Set(studioAssetIds(job))
  for (const id of await allDocumentAssetIds()) owned.delete(id)
  for (const id of await allRequestAssetIds()) owned.delete(id)
  return { briefs: 1, images: owned.size }
}

/** Studio 작업을 지운다. 작성기의 기획서는 건드리지 않는다. */
